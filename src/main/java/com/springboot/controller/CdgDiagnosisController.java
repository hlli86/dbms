...
import com.springboot.api.StoreCDGResponseMessage;
import com.springboot.api.UploadFileResponseMessage;
import com.springboot.dao.*;
import com.springboot.domain.po.*;
import com.springboot.util.FileConstants;
import com.springboot.util.FileOperations;
import com.springboot.util.Xml2Txt;
import com.springboot.util.xml2object;
...
//		}
       ...
       List<String> errorECG = null;
        ...
        for (File file : ecgFiles) {
            ...
            int patient_id = patientsMapper.findPatient_id(admissionnumber);
            String test_id = fileNameStrings[1];
            int ecg_id =ecgMapper.findId(patient_id,test_id);
            List ecg_ids=cdgMapper.findByPatientId(patient_id);
            ...
            {
                try {
                    cdgMapper.updateCdg(ecg_id,patient_id, cdg_data, thi, shi, di,result_id);
    ...
    private List<String> storeECG(String inputDir,int surgery,int ecg_result,String ecg_info) throws IOException {
        ...
        for (File file : ecgFiles) {
            ...
            List test_ids=ecgMapper.findTest_id(patient_id);
            ...
            if(test_ids.contains(test_id))
            {
                ...
                try {
                    ...
                    int id=ecgMapper.findId(patient_id,test_id);
                    ecgMapper.updateEcg(id,ecg_data,surgery,ecg_result,ecg_info);
                ...
                try {
                    ...
                    ecgMapper.insertEcg(new Ecg(patient_id,test_id,ecg_data,ecg_result,ecg_info,surgery,ecg_date));
...
//		double[][] ecgdata12 = new double[datalength][ECG_LEADS_SIZE];
    ...
    public ResponseMessage dataupload(@RequestParam("username") String username,
        ...
        while(iter.hasNext()) {
            ...
            if (case_filename.contains(FileConstants.XML_FILE_SUFFIX)) {
                ...
                if (myPatient.getAdmissionnumber() != null) {
                    Patients p = patientsMapper.findPatients(myPatient.getAdmissionnumber());
                    LOGGER.info("Admissionnumber {}.",myPatient.getAdmissionnumber());
                    if (p == null) {
                        try {
                            LOGGER.info("Starting insert Patients.");
                            patientsMapper.insertPatients(myPatient);
                            int patientId = patientsMapper.findId(myPatient.getAdmissionnumber());
                        ...
                        try {
                            ...
                            patientsMapper.updatePatients(myPatient, p.getId());
                            ecg_info = myxml2object.getEcgInfo(caseinputfile);
        ...
        for (File file : ecgFiles) {
            ...
            String[] fileNameStrings = file.getName().split("[_\\.]");
            String admissionnumber = fileNameStrings[0].length() >= 7 ? fileNameStrings[0].substring(0, 7) : fileNameStrings[0];
            int patient_id = patientsMapper.findId(admissionnumber);
            String test_id = fileNameStrings[1];
        ...
        while(it.hasNext())
        {
            Cdg cdg=cdgMapper.findByTestId(it.next().toString());
    ...
    private ResponseMessage storeCDG(String inputDir, String outputDir) throws IOException {
        ...
        for (File file : cdgFiles) {
            ...
            int ecg_id=ecgMapper.findId(patient_id,testId);
            ...
            try {
                ...
                cdgMapper.updateCdg(ecg_id,patient_id,cdgData,thi, shi,di, result_id);
